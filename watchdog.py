...
RENEW_THRESHOLD_DAYS = float(os.environ.get("RENEW_THRESHOLD_DAYS", "1"))
...
def renew_server(sb) -> bool:
    ...
    clicked = sb.execute_script(f"""
        // 策略1：直接找 title="Extend Stability" 的按钮（最精准）
        var byTitle = document.querySelector(
            'button[title="Extend Stability"], [role="button"][title="Extend Stability"]'
        );
        if (byTitle) {{
            byTitle.click();
            return 'title:Extend Stability';
        }}

        // 策略2：找包含时钟图标的按钮（fa-clock-rotate-left）
        var allBtns = document.querySelectorAll('button, [role="button"]');
        for (var i = 0; i < allBtns.length; i++) {{
            var btn = allBtns[i];
            if (btn.querySelector('.fa-clock-rotate-left, .fa-rotate-left, [class*="clock-rotate"]')) {{
                btn.click();
                return 'icon:clock-rotate';
            }}
        }}

        // 策略3：title/aria-label 包含 extend/stability/renew 关键字
        for (var i = 0; i < allBtns.length; i++) {{
            var label = (allBtns[i].title || allBtns[i].getAttribute('aria-label') || '').toLowerCase();
            if (label.includes('stability') || label.includes('extend') || label.includes('renew')) {{
                allBtns[i].click();
                return 'label:' + allBtns[i].title;
            }}
        }}
    ...
    # 检测续期是否被拒（"Too early" / "Renewal Failed" 弹窗）
    time.sleep(2)
    error_msg = sb.execute_script("""
        var body = document.body.innerText || '';
        if (/too early/i.test(body))      return 'too_early';
        if (/renewal failed/i.test(body)) return 'renewal_failed';
        if (/not enough/i.test(body))     return 'not_enough_coins';
        return 'ok';
    """)
    if error_msg != "ok":
        warn(f"续期被拒: {error_msg}")
        snap(sb, "renew-rejected")
        return False
...
def run():
    ...
    driver = Driver(
        ...
        undetectable=True,
        proxy="socks5://127.0.0.1:10808",   # Xray 本地 SOCKS5 代理，住宅 IP 绕过 Witchly 限制
